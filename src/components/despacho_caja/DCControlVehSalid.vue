    ...
    <div class="main-div mb-2">
      ...
      <div class="flex flex-row flex-wrap justify-end">
        <div class="mb-6 md:mb-0 mt-2">
          <button class="btn-save" :disabled="isFormLocked" @click="save">
            Guardar
          </button>
...
export default class DCControlVehSalid extends Vue {
  ...
  public async fetchIdPesajes(
    ...
    >;
    if (q.query) {
      res = await this.despachoCajaApi.ControlVehPdd.gedIdPesajesFind(q);
    ...
    res = await this.despachoCajaApi.ControlVehPdd.gedIdPesajes(p);
    return res.data.map<IInterfaceSelect>((d) => ({
      id: d.idPesaje,
  ...
  public async fetchFormData(): Promise<void> {
    try {
      this.isLoading = true;
      const res = await this.despachoCajaApi.ControlVehPdd.getData(
        this.idPesaje,
      );
      this.data = {
        ...res.data,
  ...
  public async onDefinitivePrint() {
    try {
      ...
      await this.despachoCajaApi.ControlVehPdd.put(this.idPesaje, this.data);
  ...
  public async save() {
    ...
    try {
      const res = await this.despachoCajaApi.ControlVehPdd.put(
        this.idPesaje,
        this.data,
      );
  ...
  private onKeydownEvent(e: KeyboardEvent) {
    switch (e.key) {
      ...
      case 'F5':
        this.data.fechaHoraS = moment().utc().format('YYY-MM-DD HH:mm:ss');
        this.data.estadoV = 'F';
