  ...
  <div class="w-full flex flex-col gap-0">
    ...
    <div class="card-table">
      ...
      <ButtonPagination
        ...
        :currentDataLength="tblDcjSubPrxDp.length"
        @click="loadMore"
      >
        Cargar más
      </ButtonPagination>
...
import { ITblDcjSubPrxDp } from '@/remote/api/despacho_caja/types';
...
@Component({
  ...
  name: 'DCCajasSubPrxDp',
})
export default class DCCajasSubPrxDp extends Vue {
  ...
  private tblDcjSubPrxDp: Array<ITblDcjSubPrxDp & { id: string }> = [];
  ...
  private async fetchData() {
    this.isLoading = true;
    try {
      const response = await this.despachoCajaApi.getDcjSubPrxDp(
        this.pagination,
      );
      this.tblDcjSubPrxDp = response.data.map((item: ITblDcjSubPrxDp, i) => ({
        ...item,
