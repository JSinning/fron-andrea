        ...
        <div class="flex flex-row flex-wrap mb-3">
          <div class="card-table h-48">
            <table class="table-fixed w-full text-xs" id="tablaC">
              ...
              <tbody
                class="text-center"
                v-for="(item, index) in tblSub3Can"
                :key="index"
              >
                <tr>
                  <td>{{ item }}</td>
...
export default class DNSubCan3DespaComponent extends Vue {
  private output = '';
  private isError: boolean = false;
  private isLoading: boolean = false;

  private tblSub3Can: any = [];

  private despaSubCan3Api = new DespachoNacionalApi(
    this.$store.getters.getAuthToken,
  );

  private async mounted() {
    await this.searchSubCan3();
  }
  ...
  private async searchSubCan3() {
    try {
      const response = await this.despaSubCan3Api.getSub3Can();
      this.tblSub3Can = response.data;
    } catch (e) {
