        ...
        <div class="flex flex-row flex-wrap mb-3">
          <div class="card-table h-48">
            <table class="table-fixed w-full text-xs" id="tablaC">
              ...
              <tbody
                class="text-center"
                v-for="(item, index) in tblSub4Can"
                :key="index"
              >
                <tr>
                  <td>{{ item }}</td>
...
export default class DNSubCan4DespaComponent extends Vue {
  private output = '';
  private isError: boolean = false;
  private isLoading: boolean = false;

  private tblSub4Can: any = [];

  private despaNacSubApi = new DespachoNacionalApi(
    this.$store.getters.getAuthToken,
  );

  private async mounted() {
    await this.searchSubCan4();
  }
  ...
  private async searchSubCan4() {
    try {
      const response = await this.despaNacSubApi.getSub4Can();
      this.tblSub4Can = response.data;
    } catch (e) {
      Sentry.captureException(e);
      this.showOutput(true, 'Error al Cargar DNSub4Can');
    }
  }
}
</script>
