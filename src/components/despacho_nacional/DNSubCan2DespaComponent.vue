        ...
        <div class="flex flex-row flex-wrap mb-3">
          <div class="card-table h-48">
            <table class="table-fixed w-full text-xs" id="tablaC">
              ...
              <tbody
                class="text-center"
                v-for="(item, index) in tblSub2RCan"
                :key="index"
              >
                <tr>
                  <td>{{ item }}</td>
...
export default class DNSubCan2DespaComponent extends Vue {
  private output = '';
  private isError: boolean = false;
  private isLoading: boolean = false;

  private tblSub2RCan: any = [];

  private DespaSubCan2Api = new DespachoNacionalApi(
    this.$store.getters.getAuthToken,
  );

  private async mounted() {
    await this.searchSubCan2();
  }
  ...
  private async searchSubCan2() {
    try {
      const response = await this.DespaSubCan2Api.getSub2Can();
      this.tblSub2RCan = response.data;
    } catch (e) {
      Sentry.captureException(e);
      this.showOutput(true, 'Error al Cargar DNSub2Can');
    }
  }
}
</script>
