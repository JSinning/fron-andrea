        ...
        <div class="flex flex-row flex-wrap mb-3">
          <div class="card">
            ...
            <div class="flex flex-row flex-wrap">
              ...
              <div class="px-3 mb-6 md:mb-0">
                <label class="label">Anualar:</label>
                <input
                  type="checkbox"
                  class="checkBox"
                  v-model="anular"
...
export default class MovDespaNacComponent extends Vue {
  private ports: any[] = [];
  ...
  private async saveRegister() {
    ...
    try {
      ...
      Sentry.captureException(e);
      this.showOutput(true, 'Error al guiardar informacion');
    }
  }
  ...
  private async addTablainfo() {
    try {
      ...
      Sentry.captureException(e);
      this.showOutput(true, 'error al guardar Reguistro');
    }
  }
  ...
  private async afterUpdateTipoMov() {
    ...
    try {
      ...
      Sentry.captureException(e);
      this.showOutput(true, 'Error al generar el Relacion de canal');
    }
  }
  ...
  private async movimientos(tipoMov: number) {
    try {
      ...
      Sentry.captureException(e);
      this.showOutput(true, 'error al buscar moviemiento');
    }
  }
  ...
  private async afterUpdateCConduct() {
    try {
      ...
      Sentry.captureException(e);
      this.showOutput(true, 'Error al generar Placa del veiculo ');
    }
  }
  ...
  private async deleteRegister() {
    ...
    if (!(await this.searchRegister())) {
      this.showOutput(true, 'El dato que desa borrar no exite');
      return;
    }
  ...
  private async searchRegister() {
    try {
      ...
      Sentry.captureException(e);
      this.showOutput(true, 'Error al ejecuter');
    }
  }
  ...
  private async deleteDespaBac(idConcecutivo: number) {
    ...
    try {
      ...
      Sentry.captureException(e);
      this.showOutput(true, 'Erro al elininar Reguistro');
    } finally {
      this.isLoading = false;
    }
  }
  ...
  private async deleteTableDespa(tipoMov: number, consecTip: number) {
    ...
    try {
      ...
      Sentry.captureException(e);
      this.showOutput(true, 'Erro al elininar Reguistro de la tabla');
    } finally {
      this.isLoading = false;
    }
  }
