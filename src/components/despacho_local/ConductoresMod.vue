...
export default class ConductoresMod extends Vue {
  ...
  private async updateConductores() {
    try {
      const response = await this.despachoLocalApi.putConductores(
        this.Iconductores,
        this.nitConduct,
      );
  ...
  private async afterBuscarClientes(nitConduc: string) {
    try {
      const response =
        await this.despachoLocalApi.getAfterBuscarClienteConductore(nitConduc);
  ...
  private async buscarClientes(p?: IPagination, q?: IQuery) {
    ...
    >;
    if (q.query !== '') {
      response = await this.despachoLocalApi.getFindClientesConductores(q);
    ...
    response = await this.despachoLocalApi.getBuscarClienteConductores(p);
    return response.data.map<IInterfaceSelect>(
      (e: Pick<IConductores, 'nitConduc' | 'nombreCond'>) => ({
        id: e.nitConduc,
        item: `${e.nombreCond} - ${e.nitConduc}`,
