    ...
    <div class="main-div mb-2">
      <div class="flex flex-row flex-wrap mb-2">
        <div class="px-3 mt-2 mb-6 md:mb-0">
          <h1 class="title">Detalle Pieles</h1>
        </div>
      </div>
...
@Component({
  ...
  name: 'ConfirmaWebSacDt2',
})
export default class ConfirmaWebSacDt2 extends Vue {
  ...
  private async mounted() {
    await this.confirmaWebSacD2(this.nGuia);
    this.datainfo();
  }
  ...
  private async confirmaWebSacD2(nGuia: number) {
    this.isLoading = true;
    try {
      const response = await this.guiaSacrificioApi.getConfirmaWebSacD2(nGuia);
