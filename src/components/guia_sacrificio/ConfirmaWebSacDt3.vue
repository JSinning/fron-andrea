    ...
    <div class="main-div mb-2">
      <div class="flex flex-row flex-wrap mb-2">
        <div class="px-3 mt-2 mb-6 md:mb-0">
          <h1 class="title">Detalle Viceras</h1>
        </div>
      </div>
...
@Component({
  ...
  name: 'ConfirmaWebSacDt3',
})
export default class ConfirmaWebSacDt3 extends Vue {
  ...
  private async mounted() {
    await this.confirmaWebSacD3(this.nGuia);
    this.datainfo();
  }
  ...
  private async confirmaWebSacD3(nGuia: number) {
    this.isLoading = true;
    try {
      const response = await this.guiaSacrificioApi.getConfirmaWebSacD3(nGuia);
