          ...
          <div class="overflow-x-hidden overflow-x-scroll">
            <table class="table-fixed w-full text-xs" id="tableA">
              ...
              <tbody
                ...
                v-for="(item, index) in tblConfirmaWeb"
                :key="index"
            ...
            <div class="content-center mt-2">
              <div class="flex flex-row flex-wrap">
                <div
                  class="rounded bg-andrea text-white px-4 mt-1 py-2 w-4/1 m-auto mb-2"
                >
                  <button class="btn-succes" @click="acceptModalActualizarW()">
                    Si
                  </button>
                </div>
                <div
...
export default class ConfirmaWeb extends Vue {
  ...
  private convertBoolean(data: boolean) {
    return data ? 'SI' : 'NO';
  }
  ...
  private totalPesoPrmedioR(totalKilosP: number, noAnimales: number) {
    ...
    return Math.round((promedioR + Number.EPSILON) * 100) / 100;
  }
  ...
  private async confirmaWeb() {
    this.isLoading = true;
    try {
      const response = await this.guiaSacrificioApi.getCofirmaWeb();
      this.tblConfirmaWeb = response.data;
    } catch (e) {
  ...
  private async actualizarW(nGuia: number) {
    this.isLoading = true;
    try {
      const response = await this.guiaSacrificioApi.putActualizarW(nGuia);
      if (response.statusCode === 200) {
        this.showOutput(false, response.message);
      }
    } catch (e) {
