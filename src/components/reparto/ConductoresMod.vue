...
export default class ConductoresMod extends Vue {
  ...
  private async updateConductores() {
    try {
      const response = await this.repartoApi.conductores.putConductores(
        this.conductores,
        this.nitConduct,
      );
      ...
      this.nitConduct = this.conductores.nitConduc;
    } catch (e) {
      captureException(e);
      this.showOutput(true, e.message);
    }
  ...
  private async afterBuscarClientes(nitConduc: string) {
    try {
      const response =
        await this.repartoApi.conductoresMod.getAfterBuscarClienteConductore(
          nitConduc,
        );
      this.conductores = response.data;
      this.conductores.fecha = convertDate(this.conductores.fecha);
      this.readonly = true;
      this.search = false;
    } catch (e) {
      captureException(e);
  ...
  private async buscarClientes(p?: IPagination) {
    const response =
      await this.repartoApi.conductoresMod.getBuscarClienteConductores(p);
